#!/bin/bash
# -------------------------------
# STEP 0: Color Configuration & Helper Functions
# -------------------------------
...
<VirtualHost *:80>
  ServerName rocky-08-01.vgs.com

  ## CORS Headers
  Header always set Access-Control-Allow-Origin "https://rocky-08-04.vgs.com"
...
<VirtualHost *:443>
  ...
  <Location "/pulpcore_registry/v2/">
    RequestHeader unset REMOTE-USER
    RequestHeader unset REMOTE_USER
    RequestHeader set REMOTE-USER "admin" "expr=%{SSL_CLIENT_S_DN_CN} == 'rocky-08-01.vgs.com'"
...
# Setup internal remote formatting colors
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[0;33m'
BLUE='\033[0;34m'
...
openssl req -x509 -newkey rsa:2048 -nodes -keyout /root/foreman-frontend/ssl/server.key -out /root/foreman-frontend/ssl/server.crt -days 365 -subj "/O=VGS/OU=VGS/CN=rocky-08-04.vgs.com"
...
REACT_APP_API_URL=https://rocky-08-01.vgs.com/api/v2
...
const API = axios.create({
  baseURL: process.env.REACT_APP_API_URL || "https://rocky-08-01.vgs.com/api/v2",
  auth: {
...
module.exports = function (app) {
  app.use(
    ...
    createProxyMiddleware({
      target: 'https://rocky-08-01.vgs.com',
      changeOrigin: true,
      secure: false
    })
  );
...
# App.js
print_header "Creating App.js..." "info"
...
print_header "Installing Nginx" "info"
dnf install nginx -y

print_header "Enabling and starting Nginx" "info"
systemctl enable nginx
systemctl start nginx

print_header "Creating SSL directory" "info"
mkdir -p /etc/nginx/ssl

print_header "Copying SSL certificates" "info"
...
print_header "Writing Nginx config: /etc/nginx/conf.d/foreman-api.conf" "info"
cat << 'NGINX' > /etc/nginx/conf.d/foreman-api.conf
# Redirect port 3000 → 443
server {
    listen 3000 ssl;
    server_name rocky-08-04.vgs.com;

    ssl_certificate     /etc/nginx/ssl/rocky.crt;
    ssl_certificate_key /etc/nginx/ssl/rocky.key;

    return 301 https://rocky-08-04.vgs.com$request_uri;
}
...
server {
    ...
    # -------------------------------
    location /api/ {
        proxy_pass https://rocky-08-01.vgs.com/api/;
...
print_header "Testing Nginx configuration" "info"
nginx -t

print_header "Restarting Nginx" "info"
systemctl restart nginx
systemctl status nginx --no-pager

print_header "Building Foreman frontend" "info"
cd /root/foreman-frontend/
npm install
npm run build

print_header "Deploying frontend to /var/www/foreman-frontend" "info"
mkdir -p /var/www/foreman-frontend
cp -r /root/foreman-frontend/build/* /var/www/foreman-frontend/

print_header "Setting permissions" "info"
chown -R nginx:nginx /var/www/foreman-frontend
chmod -R 755 /var/www/foreman-frontend

print_header "Final Nginx reload" "info"
nginx -t
systemctl restart nginx
systemctl status nginx --no-pager
...
print_header "All copied." "success"
EOF

# -------------------------------
# STEP 3: Transfer Remote Script
# -------------------------------
print_header "Transferring script..." "info"
sshpass -p 'Root@123' scp -o StrictHostKeyChecking=no /tmp/foreman_frontend.sh root@rocky-08-04.vgs.com:/root/

# -------------------------------
# STEP 4: Execute Remote Script
# -------------------------------
print_header "Executing remote script..." "info"
sshpass -p 'Root@123' ssh -o StrictHostKeyChecking=no root@rocky-08-04.vgs.com "bash /root/foreman_frontend.sh"
