...
patch_remote_node() {
    ...
    progress_stage "$NODE" "Checking CIFS Support" "10"
	...
	if [ $? -ne 0 ]
    then
        update_progress "$NODE" "CIFS Package Failed" "100" "FAILED"
        return 1
    fi
    ...
    mount -t cifs ${REPO_MOUNT} ${MOUNT_POINT} \
        -o username=${CIFS_USER},password='${CIFS_PASS}',vers=3.0,noperm
    ...
    remote_exec "$NODE" "
        test -f ${MOUNT_POINT}/centos/repodata/repomd.xml &&
        test -f ${MOUNT_POINT}/installed_rhel7/repodata/repomd.xml
    "
...
remote_exec "$NODE" "
    test -f /etc/yum.repos.d/centos7.repo &&
...
"
    ...
    remote_exec "$NODE" "
        yum makecache fast \
        --disablerepo='*' \
        --enablerepo=centos7,rhel7-installed
    "
    ...
    progress_stage "$NODE" "Installing Updates" "70"
    ...
    then
        update_progress \
            "$NODE" \
            "Patch Installed - Restore Failed" \
            "90" \
            "FAILED"
        return 1
    fi
