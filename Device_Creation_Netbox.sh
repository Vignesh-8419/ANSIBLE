...
if [ "$CLUSTER_MODE" == "2" ]; then
    ...
    read -p "Enter Cluster Group name: (eg: rocky-8-server / centos-07-servers)" GROUP_NAME
    GROUP_ID=$(get_or_create "virtualization/cluster-groups" "$GROUP_NAME")

    read -p "Enter Cluster name: (eg: rocky-8-server / centos-07-servers)" CLUSTER_NAME
