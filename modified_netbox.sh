...
# 🔧 Completely disable the default nginx.conf server block
echo "[10.1/14] Disabling built-in nginx server block..."
sed -i '/server_name[[:space:]]\+_;/,/}/ s/^/#/' /etc/nginx/nginx.conf
sed -i 's/listen[[:space:]]\+80[[:space:]]*;/#listen 80;/' /etc/nginx/nginx.conf
sed -i 's/listen[[:space:]]*\[::\]:80.*;/#listen [::]:80;/' /etc/nginx/nginx.conf
