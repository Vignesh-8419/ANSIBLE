...
{
REPO="$1"
$HAMMER repository info --organization "${ORG}" --product "${PRODUCT_NAME}" --name "${REPO}" 2>/dev/null | awk -F':' '/^Id/ {gsub(/ /,"",$2);print $2}'
}
###############################################################################
# Create Product
###############################################################################
...
if $HAMMER product info --organization "${ORG}" --name "${PRODUCT_NAME}" >/dev/null 2>&1
then
skip "Product ${PRODUCT_NAME} already exists."
else
info "Creating Product ${PRODUCT_NAME}"
$HAMMER product create --organization "${ORG}" --name "${PRODUCT_NAME}"
if [ $? -eq 0 ]
then
ok "Product created."
else
...
if $HAMMER repository info --organization "${ORG}" --product "${PRODUCT_NAME}" --name "${REPO_NAME}" >/dev/null 2>&1
then
skip "${REPO_NAME} already exists."
else
info "Creating Repository : ${REPO_NAME}"
$HAMMER repository create --organization "${ORG}" --product "${PRODUCT_NAME}" --name "${REPO_NAME}" --content-type yum --url "${REPO_URL}"
if [ $? -eq 0 ]
then
...
CURRENT_URL=$($HAMMER repository info --organization "${ORG}" --product "${PRODUCT_NAME}" --name "${REPO_NAME}" 2>/dev/null | awk -F':' '/URL/ {print $2}' | xargs)
info "Current URL : ${CURRENT_URL}"
info "Required URL : ${REPO_URL}"
$HAMMER repository update --organization "${ORG}" --product "${PRODUCT_NAME}" --name "${REPO_NAME}" --url "${REPO_URL}"
if [ $? -eq 0 ]
then
...
create_repo "${BASE_REPO}" "${BASE_URL}"
create_repo "${UPDATE_REPO}" "${UPDATE_URL}"
create_repo "${ELEVATE_REPO}" "${ELEVATE_URL}"
header "Updating Repository URLs"
update_repo_url "${BASE_REPO}" "${BASE_URL}"
update_repo_url "${UPDATE_REPO}" "${UPDATE_URL}"
update_repo_url "${ELEVATE_REPO}" "${ELEVATE_URL}"
}
###############################################################################
...
STATUS=$($HAMMER repository info --organization "${ORG}" --product "${PRODUCT_NAME}" --name "${REPO_NAME}" 2>/dev/null | grep -Ei "Sync State|Sync Status" | awk -F':' '{print $2}' | xargs)
...
if $HAMMER content-view info --organization "${ORG}" --name "${CONTENT_VIEW}" >/dev/null 2>&1
then
skip "Content View ${CONTENT_VIEW} already exists."
else
info "Creating Content View ${CONTENT_VIEW}"
$HAMMER content-view create --organization "${ORG}" --name "${CONTENT_VIEW}"
if [ $? -eq 0 ]
then
...
EXISTING=$($HAMMER content-view info --organization "${ORG}" --name "${CONTENT_VIEW}" 2>/dev/null | grep -F "${REPO_NAME}" || true)
if [ -n "${EXISTING}" ]
then
...
$HAMMER content-view add-repository --organization "${ORG}" --name "${CONTENT_VIEW}" --repository-id "${REPO_ID}"
if [ $? -eq 0 ]
then
ok "${REPO_NAME} added."
else
...
FAILED=0
for REPO in "${BASE_REPO}" "${UPDATE_REPO}" "${ELEVATE_REPO}"
do
if $HAMMER content-view info --organization "${ORG}" --name "${CONTENT_VIEW}" 2>/dev/null | grep -Fq "${REPO}"
then
...
ACTIVE_TASK=$($HAMMER task list --search "state = running" 2>/dev/null | grep -F "${CONTENT_VIEW}" || true)
if [ -n "${ACTIVE_TASK}" ]
then
...
OUTPUT=$($HAMMER content-view publish --organization "${ORG}" --name "${CONTENT_VIEW}" --description "EL7 Migration Publish $(date '+%F %T')" 2>&1)
RC=$?
echo "${OUTPUT}"
if [ ${RC} -eq 0 ]
then
...
if $HAMMER activation-key info --organization "${ORG}" --name "${ACTIVATION_KEY}" >/dev/null 2>&1
then
skip "Activation Key ${ACTIVATION_KEY} already exists."
info "Updating Activation Key Content View"
$HAMMER activation-key update --organization "${ORG}" --name "${ACTIVATION_KEY}" --content-view "${CONTENT_VIEW}" --lifecycle-environment "Library"
if [ $? -eq 0 ]
then
...
$HAMMER activation-key create --organization "${ORG}" --name "${ACTIVATION_KEY}" --content-view "${CONTENT_VIEW}" --lifecycle-environment "Library"
if [ $? -eq 0 ]
then
...
$HAMMER activation-key info --organization "${ORG}" --name "${ACTIVATION_KEY}"
if [ $? -eq 0 ]
then
...
echo "Product"
echo "============================================================"
$HAMMER product info --organization "${ORG}" --name "${PRODUCT_NAME}"
echo
echo "============================================================"
echo "Repositories"
echo "============================================================"
$HAMMER repository list --organization "${ORG}" --product "${PRODUCT_NAME}"
echo
echo "============================================================"
echo "Content View"
echo "============================================================"
$HAMMER content-view info --organization "${ORG}" --name "${CONTENT_VIEW}"
echo
echo "============================================================"
echo "Activation Key"
echo "============================================================"
