...
echo "Creating REPOS"
  ...
  cat <<REPO > /etc/yum.repos.d/base.repo
[base]
...
REPO
  ...
  cat <<REPO > /etc/yum.repos.d/foreman-new.repo
[foreman-new-repo]
...
REPO
  ...
  cat <<REPO > /etc/yum.repos.d/foreman.repo
[foreman]
...
echo "Creating directory and mounting ISO"

mkdir -p /var/www/html/repo
mount -t cifs //192.168.31.87/ISO /var/www/html/repo -o username=vigne,password='Vigneshv12$',ro,vers=3.0

echo "Created directory and mounting ISO"

echo "Creating DNS config"

  cat <<'RESOLVE' > /etc/resolv.conf
# Generated by NetworkManager
search vgs.com
nameserver 8.8.8.8
RESOLVE

echo "created DNS config"
...
echo "Config PIHOLE file"
  ...
  cat <<'PIHOLE' > /root/Pi-hole/automated\ install/basic-install.sh
#!/usr/bin/env bash
...
install_dependent_packages() {
    ...
    # Install Fedora/CentOS packages
          ...
          if rpm -q pihole-meta &>/dev/null; then
              printf "%b  %b %s\\n" "${OVER}" "${TICK}" "Pi-hole dependency package already installed"
          elif [ -f /tmp/pihole-meta.rpm ]; then
              if yum localinstall -y /tmp/pihole-meta.rpm &>/dev/null; then
                  printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}"
...
echo "installed epel release package"
